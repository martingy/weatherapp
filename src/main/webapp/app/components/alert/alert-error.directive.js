(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('weatherappApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-weatherappApp-error');
                var entityKey = httpResponse.headers('X-weatherappApp-params');
                if (errorHeader) {
